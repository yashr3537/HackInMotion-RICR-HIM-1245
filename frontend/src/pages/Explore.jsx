...
export default function Explore() {
  ...
  const isLocationSaved = (loc) => {
    return savedLocations.some(
      (saved) =>
        ...
        (saved.name &&
          loc.name &&
          saved.name.toLowerCase() === loc.name.toLowerCase() &&
          saved.region === loc.region)
    )
  }
    ...
    if (alreadySaved) {
      ...
      try {
        ...
        if (success) {
          ...
          setToast({
            type: 'error',
            text: `Failed to remove "${location.name}". Please try again.`,
          })
        }
      } catch (err) {
  ...
  const selectLocation = (location) => {
    ...
    localStorage.setItem('selectedAirGuardLocation', JSON.stringify(selectedLocation))
    ...
    <div className="flex flex-col gap-6 relative">
          ...
          {results.map((loc) => (
            ...
            >
              ...
              <div className="flex items-start justify-between">
                ...
                {(() => {
                  ...
                  const isActionLoading =
                    actionLoadingId === (loc.id || `${loc.name}-${loc.latitude}`)
                  return (
                    <button
                      type="button"
                      ...
                      aria-label={
                        saved
                          ? `Remove ${loc.name} from My Locations`
                          : `Save ${loc.name} to My Locations`
                      }
                      title={
                        saved
                          ? `Remove ${loc.name} from My Locations`
              ...
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-ink-400">Current AQI</p>

                  <p className="font-mono font-bold text-2xl text-ink-900 mt-1">{loc.aqi ?? '—'}</p>
                </div>

                {loc.aqi !== null && loc.aqi !== undefined && <RiskBadge aqi={loc.aqi} size="sm" />}
              </div>

              {/* POLLUTANTS */}
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-lg bg-ink-50 p-2">
                  <p className="text-ink-400">PM2.5</p>

                  <p className="font-semibold text-ink-700">{loc.pm25 ?? '—'}</p>
                </div>

                <div className="rounded-lg bg-ink-50 p-2">
                  <p className="text-ink-400">PM10</p>

                  <p className="font-semibold text-ink-700">{loc.pm10 ?? '—'}</p>
                </div>
              </div>

              <p className="text-[10px] text-ink-400">Live data from Open-Meteo</p>
      ...
      {!query.trim() && !loading && (
        <div className="rounded-xl border border-dashed border-ink-200 bg-surface p-8 text-center">
          <MapPin size={28} className="mx-auto text-forest-600 mb-3" />

          <p className="font-semibold text-ink-800">Search for a location</p>
