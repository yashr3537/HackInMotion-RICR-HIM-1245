export default {
  common: {
    search: 'തിരയുക',
    save: 'സേവ്',
    cancel: 'റദ്ദ് ചെയ്യുക',
    close: 'മൂടുക',
    back: 'മടങ്ങുക',
    next: 'അടുത്തത്',
    viewDetails: 'വിവരങ്ങൾ കാണുക',
    checkAirQuality: 'കാറ്റിന്റെ ഗുണനിലവാരം പരിശോധിക്കുക',
    language: 'ഭാഷ',
    live: 'ലाइव',
    monitoring: 'മോണിറ്ററിംഗ്',
  },

  nav: {
    home: 'ഹോം',
    features: 'പ്രാധാന്യങ്ങള',
    explore: 'പരിശോധിക്കുക',
    howItWorks: 'ഇത് എങ്ങനെ പ്രവർത്തിക്കുന്നു',
    about: 'ഞങ്ങളെക്കുറിച്ച്',
    dashboard: 'ഡാഷ్బോർഡ്',
    alerts: 'എലർട്ടുകൾ',
    locations: 'എന്റെ സ്ഥലങ്ങൾ',
    history: 'ചരിത്രം',
    activity: 'പ്രവർത്തന അപകടം',
    routeRisk: 'റൂട്ട് റിസ്ക്',
    compare: 'തോർക്കുക',
    reportIssue: 'പ്രശ്നം റിപ്പോർട്ട് ചെയ്യുക',
    myReports: 'എന്റെ റിപ്പോർട്ടുകൾ',
    profile: 'പ്രൊഫൈൽ',
    settings: 'സജ്ജീകരണങ്ങൾ',
  },

  dashboard: {
    welcomeMorning: 'സുപ്രഭാതം',
    welcomeAfternoon: 'സുപ്രഭാത സമയം',
    welcomeEvening: 'ശുഭ സന്ധ്യ',
    pollutantBreakdown: 'ദൂഷ്യക വിഭാഗീകരണം',
    myLocations: 'എന്റെ സ്ഥലങ്ങൾ',
    recentAlerts: 'സമീപകാല എലർട്ടുകൾ',
  },

  settings: {
    title: 'സജ്ജീകരണങ്ങൾ',
    language: 'ഭാഷ',
    voiceAlerts: 'വോയ്‌സ് അലർട്ടുകൾ',
    darkMode: 'ഡാർക്ക് മോഡ്',
    applicationPreferences: 'ആപ്ലിക്കേഷൻ മുൻഗണനകൾ',
  },

  alerts: {
    title: 'നിങ്ങളുടെ വായു എലർട്ടുകൾ',
    critical: 'ഗൗരവമേറിയ',
    warning: 'എಚ್ಚരവൈ',
    information: 'വിവരം',
    aqiMessage: '{location} നു വേണ്ടി വായു നിലവാരം മുന്നറിയിപ്പ്. നിലവിലെ AQI {aqi}.',
  },

  aqi: {
    good: 'നല്ലത്',
    moderate: 'മध്യസ്ഥം',
    sensitive: 'സെന്‍സറ്റീവ് ഗ്രൂപ്പുകൾക്ക് അനുകൂലമല്ല',
    unhealthy: 'ആരോഗ്യത്തിന് ഹാനികരമാകാം',
    hazardous: 'ഹാനികരമായ',
  },

  pollutant: {
    relativeLevel: 'ആപേക്ഷിക നിലവാരം',
    environmentalMetric: 'പരിസ്ഥിതി മാനദണ്ഡം',
  },

  recommendation: {
    header: 'നിങ്ങളുടെ പരിസ്ഥിതി ശുപാർശ',
    personalized: 'പരിസരനിർവചിതം',
    profile: 'പ്രൊഫൈൽ',
    activity: 'പ്രവർത്തനം',
    basedOn: 'നിലവിലെ പരിസ്ഥിതി നിലകളുടെ അടിസ്ഥാനത്തിൽ',
    checkActivity: 'പ്രവർത്തന അപകടം പരിശോധിക്കുക',
  },

  dominant: {
    main: 'പ്രധാന മലിനീകരണ ആശങ്ക',
    currentMetric: 'നിലവിലെ പ്രധാന പരിസ്ഥിതി മാനദണ്ഡം',
    dominantPollutant: 'പ്രധാന മലിനീകരണം',
    exposureRelative: 'പരിധിയോടനുബന്ധിച്ചാണ് കോപ്പുക',
    reference: 'റഫറൻസ്',
    higherAttention: 'ഉയർന്ന ശ്രദ്ധാ നിർദ്ദേശം',
    monitorConditions: 'നിലവിലെ സാഹചര്യങ്ങൾ നിരീക്ഷിക്കുക',
    withinRange: 'നിലവിലെ തല പ്രതീക്ഷിക്കുന്ന പരിധിയിലാണി',
    indicatorInfo:
      'ഈ സൂചിക നിലവിലെ മലിനീകരണ നിലക്കും അതിന്റെ സജ്ജീകരിച്ച റഫറൻസ് പരിധിക്കും അടിസ്ഥാനമാക്കിയുള്ളതാണ്.',
  },


  landing: {
    realtimeIntelligence: 'റியல்-ടൈം പരിസ്ഥിതി ബുദ്ധിമുട്ട്',
    heroTitlePart1: 'വായു അറിയൂ',
    heroTitlePart2: 'നിങ്ങളുടെ ചുറ്റുപാടുകളിൽ.',
    heroDescription:
      'സങ്കീർണ്ണ പരിസ്ഥിതി ഡാറ്റയെ വ്യക്തിഗതമായ, വ്യക്തമായ നടപടികളിലൂടെ മാറ്റുക. നിങ്ങളുടെ വായു ഗുണനം, അപകടം, അടുത്തെന്ത് ചെയ്യണമെന്ന് മനസിലാക്കുക.',
    checkAirQuality: 'വായു ഗുണനിലവാരം പരിശോധിക്കുക',
    exploreFeatures: 'ഫീച്ചറുകൾ പരിശോധിക്കുക',
    liveEnvironmentalData: 'ലൈവ് പരിസ്ഥിതി ഡാറ്റ',
    clearRiskGuidance: 'സ്പഷ്ടമായ അപകട മാർഗ്ഗനിർദേശം',
    locationAlerts: 'സ്ഥലം അലർട്ട്',
    liveAirQuality: 'ലൈവ് വായു ഗുണനിലവാരം',
    environmentalStatus: 'പരിസ്ഥിതി നില',
    liveMonitoring: 'ലൈവ് നിരീക്ഷണം',
    features: 'ഫീച്ചറുകൾ',
    featuresHeadline: 'നിങ്ങളുടെ വായു മനസ്സിലാക്കാൻ ആവശ്യമായ എല്ലാം',
    featuresDescription:
      'ലീവ് സ്ഥിതികളിൽ നിന്ന് ചരിത്ര ട്രെൻഡുകൾക്കും വ്യക്തിഗത മാർഗ്ഗനിർദേശങ്ങൾക്കുമുള്ളവ까지, AirGuard വിവരങ്ങൾ ലളിതവും പ്രായോഗികവുമാണ്.',
    exploreCapability: 'ശക്തി പരിശോധിക്കുക',
    howItWorksTitle: 'ഇത് എങ്ങനെ പ്രവർത്തിക്കുന്നു',
    howItWorksHeadline: 'സ്ഥലത്തു നിന്ന് പ്രവർത്തി വരെ, നാല് ഘട്ടങ്ങളിൽ',
    premiumAwareness: 'പരിസ്ഥിതി ജാഗ്രത, വ്യക്തിഗത',
    builtForPeople: 'പ്രതിദിനം sama മേൽശ്വാസമുള്ള ആളുകൾക്ക് നിർമിച്ചതാണ്.',
    premiumDescription:
      'AirGuard ഒരു ഹാക്കത്തോൺ പ്രോജക്ടായി നിർമ്മിച്ചിരിക്കുന്നു, പരിസ്ഥിതിവാരിയം വ്യക്തിഗതവും വിശദവുമായും പ്രവർത്തനക്ഷമവുമായും മാറ്റാൻ.',
    ctaCheckAirQuality: 'വായു ഗുണനിലവാരം പരിശോധിക്കുക',
  },

  features: {
    live: {
      title: 'ലൈവ് വായു ഗുണനിലവാരം',
      description: 'ഏത് പ്രാവശ്യമായതും റിയൽ-ടൈം വായു ഗുണനിലവാരം നിരീക്ഷിക്കുക.',
    },
    risk: {
      title: 'റിസ്‌ക് ഇന്റലിജൻസ്',
      description:
        'സങ്കീർണ്ണ മാലിന്യ ഡേറ്റയെ എളുപ്പം മനസ്സിലാക്കാവുന്ന റിസ്‌ക് ലെവലാക്കായി പരിഷ്കരിക്കുക.',
    },
    guidance: {
      title: 'വ്യക്തിഗത മാർഗ്ഗനിർദേശം',
      description: 'നിങ്ങളുടെ പരിസ്ഥിതി സെൻസിറ്റിവിറ്റി അടിസ്ഥാനമാക്കിയുള്ള ശുപാർശകൾ ലഭിക്കുക.',
    },
    trends: {
      title: 'ചരിത്ര ട്രെൻഡുകൾ',
      description: 'വായു ഗുണനിലവാരം മെച്ചപ്പെടുന്നുണ്ടോ അല്ലെങ്കിൽ പോവുകയാണോ എന്ന് മനസ്സിലാക്കുക.',
    },
    alerts: {
      title: 'സ്മാർട്ട് അലർട്ടുകൾ',
      description: 'നിങ്ങൾ സംരക്ഷിച്ച സ്ഥലങ്ങൾ എപ്പോൾ അപകടത്തിലാകും എന്ന് അറിയുക.',
    },
    activity: {
      title: 'പ്രവൃത്തി റിസ്‌ക് ഉപദേധാവ്',
      description:
        'ഇപ്പോഴുള്ള ഓടൽ, സൈക്ലിംഗ് അല്ലെങ്കിൽ പുറത്തുള്ള പ്രവർത്തനങ്ങൾ യോജിച്ചതാണോ എന്ന് പരിശോധിക്കുക.',
    },
  },

  howItWorks: {
    step1: {
      title: 'സ്ഥലം തിരഞ്ഞെടുക്കുക',
      description:
        'തുടങ്ങാൻ ഏതെങ്കിലും നഗരത്തെ അന്വേഷിക്കുക അല്ലെങ്കിൽ നിങ്ങളുടെ നിലവിലെ സ്ഥലം ഉപയോഗിക്കുക.',
    },
    step2: {
      title: 'ലൈവ് ഡാറ്റ നേടുക',
      description: 'AirGuard ആ സ്ഥലത്തിനുള്ള ഏറ്റവും പുതിയ മലിനത വായനകൾ എടുക്കും.',
    },
    step3: {
      title: 'നിങ്ങളുടെ റിസ്‌ക് മനസ്സിലാക്കുക',
      description:
        'നിങ്ങളുടെ പരിസ്ഥിതി പ്രൊഫൈലിന്റെ അടിസ്ഥാനത്തിൽ ഒരു വ്യക്തമായ റിസ്‌ക് ലെവൽ കാണുക.',
    },
    step4: {
      title: 'നടപടികൾ സ്വീകരിക്കുക',
      description:
        'പ്രവൃത്തി, പ്രതിരോധം, മാനദണ്ഡങ്ങളെക്കുറിച്ച് വ്യക്തിഗത മാർഗ്ഗനിർദേശങ്ങൾ പിന്തുടരുക.',
    },
  },

  sidebar: {
    footerNotice:
      'ഗాలి ഗുണനിലവാരം ഡാറ്റ സ്വയം പുതുക്കും. ഡെമോ ബിൽഡ് — ബാക്ക്‌എൻഡ് ഇനിയും ബന്ധിപ്പിച്ചിട്ടില്ല.',
  },

  auth: {
    login: 'Login',
    signup: 'Sign up',
    forgotPassword: 'Forgot your password?',
    forgotPasswordDescription: 'Enter the email associated with your AirGuard account.',
    sendResetLink: 'Send reset link',
    resetPassword: 'Reset password',
    resetPasswordDescription: 'Choose a new password for your account.',
    newPassword: 'New password',
    confirmNewPassword: 'Confirm new password',
    passwordUpdated: 'Password updated successfully',
    passwordResetSuccess: 'Your AirGuard password has been changed.',
    invalidResetToken: 'Invalid or missing reset token.',
    resetTokenExpired: 'Reset token has expired.',
    backToSignIn: 'Back to Sign In',
    emailRequired: 'Email is required.',
    invalidEmail: 'Please enter a valid email address.',
    passwordsDoNotMatch: 'Passwords do not match.',
    emailAddress: 'Email address',
    emailPlaceholder: 'you@example.com',
    sending: 'Sending…',
    demoResetCreated: 'Demo reset created',
    demoResetNotice:
      'For this demo the reset flow is simulated locally. Use the link below to complete the reset.',
    openResetLink: 'Open reset link',
    resetRequestFailed: 'Unable to create reset request. Please try again.',
    passwordRequired: 'Password is required.',
    passwordTooShort: 'Password must be at least 6 characters.',
    resetFailed: 'Unable to reset password. The token may be invalid or expired.',
    newPasswordPlaceholder: 'Enter a new password',
    confirmNewPasswordPlaceholder: 'Confirm new password',
    userNotFound: 'No account found with that email address.',
    resetTokenUsed: 'This reset token has already been used.',
  },

  report: {
    title: 'പരിസ്ഥിതി പ്രശ്നം റിപ്പോർട്ട് ചെയ്യുക',
    subtitle: 'നിങ്ങളുടെ പ്രദേശത്തെ പരിസ്ഥിതി പ്രശ്നങ്ങൾ രേഖപ്പെടുത്താൻ സഹായിക്കുക.',
    locationHeading: 'സ്ഥല വിവരങ്ങൾ',
    autoDetectBtn: 'എന്റെ നിലവിലെ സ്ഥാനം ഉപയോഗിക്കുക',
    detectingLocation: 'സ്ഥാനം കണ്ടെത്തുന്നു...',
    changeLocationBtn: 'സ്ഥാനം മാറ്റുക',
    manualLocationHeading: 'മാനുവൽ സ്ഥാനം',
    cityLabel: 'നഗരം / പ്രദേശം',
    regionLabel: 'സംസ്ഥാനം / മേഖല',
    addressLabel: 'വിലാസം / വിവരങ്ങൾ (ഓപ്ഷണൽ)',
    gpsDenied: 'ലൊക്കേഷൻ അനുമതി നിരസിച്ചു.',
    gpsTimeout: 'ലൊക്കേഷൻ അഭ്യർത്ഥന സമയം കഴിഞ്ഞു.',
    gpsUnsupported: 'ബ്രൗസർ ജിയോലൊക്കേഷൻ പിന്തുണയ്ക്കുന്നില്ല.',
    categoryLabel: 'പ്രശ്ന വിഭാഗം',
    selectCategory: 'വിഭാഗം തിരഞ്ഞെടുക്കുക...',
    cat_industrial_smoke: 'വ്യവസായ പുക',
    cat_vehicle_pollution: 'വാഹന മലിനീകരണം',
    cat_garbage_burning: 'മാലിന്യം കത്തിക്കൽ',
    cat_waste_dump: 'മാലിന്യ നിക്ഷേപം',
    cat_dust_pollution: 'പൊടി മലിനീകരണം',
    cat_construction_dust: 'നിർമ്മാണ പൊടി',
    cat_chemical_smell: 'രാസ മണം',
    cat_factory_emission: 'ഫാക്ടറി പുറന്തള്ളൽ',
    cat_tree_cutting: 'മരം മുറിക്കൽ',
    cat_other: 'മറ്റുള്ളവ',
    severityLabel: 'തീവ്രത നില',
    sev_low: 'കുറഞ്ഞത്',
    sev_medium: 'മിതമായത്',
    sev_high: 'ഉയർന്നത്',
    sev_critical: 'അത്യന്തം ഗുരുതരം',
    descriptionLabel: 'പ്രശ്നം വിവരിക്കുക',
    descriptionPlaceholder:
      'ദിവസവും വൈകുന്നേരം 7 മണിക്ക് അടുത്തുള്ള ഫാക്ടറിയിൽ നിന്ന് പുക വരുന്നു...',
    descriptionMinErr: 'വിവരണം കുറഞ്ഞത് 10 അക്ഷരങ്ങൾ ഉണ്ടായിരിക്കണം.',
    healthImpactLabel: 'ആരോഗ്യപരമായ ആഘാതം',
    healthNotice: 'വിവര ശേഖരണത്തിനായി മാത്രം.',
    hp_breathing_difficulty: 'ശ്വാസതടസ്സം',
    hp_allergy: 'അലർജി / കണ്ണ് നീറ്റൽ',
    hp_eye_irritation: 'കണ്ണ് നീറ്റൽ',
    hp_headache: 'തലവേദന',
    hp_cough: 'തുടർച്ചയായ ചുമ',
    hp_chest_pain: 'നെഞ്ചിൽ അസ്വസ്ഥത',
    hp_other: 'മറ്റ് ലക്ഷണങ്ങൾ',
    photoUploadLabel: 'ഫോട്ടോ തെളിവ് അപ്‌ലോഡ് ചെയ്യുക (JPEG, PNG, WEBP — max 10MB)',
    videoUploadLabel: 'വീഡിയോ തെളിവ് അപ്‌ലോഡ് ചെയ്യുക (MP4, WEBP, MOV — max 50MB)',
    chooseFile: 'ഫയൽ തിരഞ്ഞെടുക്കുക',
    removeFile: 'ഫയൽ മാറ്റുക',
    dateTimeLabel: 'സംഭവ തീയതിയും സമയവും',
    contactHeading: 'ബന്ധപ്പെടാനുള്ള വിവരങ്ങൾ',
    contactName: 'നിങ്ങളുടെ പേര്',
    contactEmail: 'ഇമെയിൽ വിലാസം',
    contactPhone: 'ഫോൺ നമ്പർ (ഓപ്ഷണൽ)',
    submitAnonymously: 'അജ്ഞാതമായി (Anonymous) സമർപ്പിക്കുക',
    anonymousNotice: 'നിങ്ങളുടെ വ്യക്തിത്വം സംരക്ഷിക്കപ്പെടും.',
    submitBtn: 'മലിനീകരണ റിപ്പോർട്ട് സമർപ്പിക്കുക',
    submittingBtn: 'സമർപ്പിക്കുന്നു...',
    successTitle: 'റിപ്പോർട്ട് വിജയകരമായി സമർപ്പിച്ചു',
    successSubtitle: 'നാടിനെ ശുചിയായി സൂക്ഷിക്കാൻ സഹായിച്ചതിന് നന്ദി.',
    reportIdLabel: 'റിപ്പോർട്ട് ഐഡി',
    statusLabel: 'ആദ്യ നില',
    viewMyReportsBtn: 'എന്റെ റിപ്പോർട്ടുകൾ കാണുക',
    backToDashboardBtn: 'ഡാഷ്‌ബോർഡിലേക്ക് മടങ്ങുക',
    newReportBtn: 'മറ്റൊരു റിപ്പോർട്ട് സമർപ്പിക്കുക',
  },
  myReports: {
    title: 'എന്റെ മലിനീകരണ റിപ്പോർട്ടുകൾ',
    subtitle: 'നിങ്ങൾ നൽകിയ റിപ്പോർട്ടുകളുടെ നില പരിശോധിക്കുക.',
    noReports: 'റിപ്പോർട്ടുകളൊന്നും ഇതുവരെ സമർപ്പിച്ചിട്ടില്ല.',
    noReportsDesc: 'മലിനീകരണ പ്രശ്നങ്ങൾ റിപ്പോർട്ട് ചെയ്ത് നിങ്ങളുടെ പ്രദേശം സംരക്ഷിക്കുക.',
    reportIssueBtn: 'പുതിയ പ്രശ്നം റിപ്പോർട്ട് ചെയ്യുക',
    colId: 'റിപ്പോർട്ട് ഐഡി',
    colLocation: 'പ്രദേശം / നഗരം',
    colCategory: 'വിഭാഗം',
    colSeverity: 'തീവ്രത',
    colStatus: 'നില',
    colDate: 'തീയതി',
    colAction: 'നടപടി',
    viewDetails: 'വിശദാംശങ്ങൾ കാണുക',
  },
  reportDetails: {
    title: 'റിപ്പോർട്ട് വിവരങ്ങൾ',
    backBtn: 'എന്റെ റിപ്പോർട്ടുകളിലേക്ക് മടങ്ങുക',
    timelineHeading: 'നില പുരോഗതി',
    locationDetails: 'സ്ഥല വിവരങ്ങൾ',
    evidenceHeading: 'ഫോട്ടോ & വീഡിയോ തെളിവുകൾ',
    noEvidence: 'മാധ്യമ തെളിവുകളൊന്നും അപ്‌ലോഡ് ചെയ്തിട്ടില്ല.',
    contactInfo: 'ബന്ധപ്പെടാനുള്ള വിവരങ്ങൾ',
    anonymousUser: 'അജ്ഞാതമായി നൽകിയത് (വിവരങ്ങൾ സംരക്ഷിതം)',
    submittedOn: 'റിപ്പോർട്ട് ചെയ്ത തീയതി',
    updatedOn: 'അവസാനം പുതുക്കിയത്',
    coordinates: 'GPS അക്ഷാംശ രേഖാംശങ്ങൾ',
  },
  status: {
    pending: 'പരിശോധന കാത്തിരിക്കുന്നു',
    under_review: 'പരിശോധനയിലാണ്',
    in_progress: 'പുരോഗതിയിൽ',
    resolved: 'പരിഹരിച്ചു',
    rejected: 'നിരസിച്ചു',
  },
}
